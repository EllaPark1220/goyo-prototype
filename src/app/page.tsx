    ...
    <main className="flex flex-col min-h-screen bg-accent-darkest text-white">
      ...
      <section className="relative min-h-screen overflow-hidden bg-[#0A3A42] flex items-center justify-center">
        ...
        <div className="relative z-30 flex flex-col items-center text-center px-6 md:px-10 max-w-[860px]">
          ...
          <div className="flex flex-col md:flex-row gap-3 opacity-0 w-full md:w-auto max-w-[280px] md:max-w-none" style={{ animation: 'fadeUp 1.4s ease-out 1.0s forwards' }}>
            <Link href="#stage-3" className="flex items-center justify-center px-8 py-4 bg-accent-main text-white font-semibold rounded-xl transition-all hover:bg-accent-deep hover:-translate-y-0.5 shadow-[0_10px_28px_-10px_rgba(0,0,0,0.5)] hover:shadow-[0_18px_40px_-14px_rgba(26,142,156,0.7)] tracking-wide">
              강의 둘러보기
            </Link>
            <Link href="/login" className="flex items-center justify-center px-8 py-4 bg-white/10 text-white font-semibold rounded-xl border-2 border-white/45 backdrop-blur-md transition-all hover:bg-white/20 hover:border-white hover:-translate-y-0.5 tracking-wide">
              로그인
            </Link>
          </div>
        </div>
      ...
      <section id="stage-3" className="relative px-6 py-20 md:px-10 md:py-36 text-white overflow-hidden" style={{
        ...
        <div className="max-w-[1100px] mx-auto relative z-10">
          <div ref={addToRefs} className="opacity-0 translate-y-8 transition-all duration-1000 ease-out text-center">
            <div className="font-mono text-[11px] tracking-[0.3em] text-white/85 uppercase font-semibold mb-5">CURRICULUM</div>
            <h2 className="font-serif font-semibold text-[28px] md:text-[min(4.2vw,44px)] leading-tight tracking-tight mb-4 text-white drop-shadow-md">5권 133편의 흐름</h2>
            <p className="font-sans text-[15px] md:text-[min(1.4vw,16.5px)] text-white/90 leading-relaxed mb-16 drop-shadow-sm">
              처음부터 끝까지 순서대로 읽어도 좋고,<br/>관심 가는 권부터 펼쳐도 좋습니다.
            </p>
          </div>

          <div ref={addToRefs} className="flex flex-col gap-3.5 max-w-[720px] mx-auto opacity-0 translate-y-8 transition-all duration-1000 ease-out delay-100">
            <div className="group bg-white/10 border border-white/20 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-2 md:gap-5 backdrop-blur-md transition-all duration-300 hover:bg-white/20 hover:translate-x-1 hover:border-white/40 cursor-pointer">
              <div className="font-serif font-semibold text-base text-white/95 tracking-tight md:w-[70px]">1권</div>
              <div className="flex-1 text-left">
                <div className="font-serif font-semibold text-lg text-white tracking-tight mb-1">돈의 언어</div>
                <div className="font-sans text-[13.5px] text-white/80">뉴스·영수증·월급명세서의 단어가 들림</div>
              </div>
              <div className="font-mono text-[11px] text-white/70 tracking-wider">27편</div>
            </div>
            <div className="group bg-white/10 border border-white/20 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-2 md:gap-5 backdrop-blur-md transition-all duration-300 hover:bg-white/20 hover:translate-x-1 hover:border-white/40 cursor-pointer">
              <div className="font-serif font-semibold text-base text-white/95 tracking-tight md:w-[70px]">2권</div>
              <div className="flex-1 text-left">
                <div className="font-serif font-semibold text-lg text-white tracking-tight mb-1">돈의 흐름</div>
                <div className="font-sans text-[13.5px] text-white/80">환율·금리 뉴스가 내 삶과 연결됨</div>
              </div>
              <div className="font-mono text-[11px] text-white/70 tracking-wider">25편</div>
            </div>
            <div className="group bg-white/10 border border-white/20 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-2 md:gap-5 backdrop-blur-md transition-all duration-300 hover:bg-white/20 hover:translate-x-1 hover:border-white/40 cursor-pointer">
              <div className="font-serif font-semibold text-base text-white/95 tracking-tight md:w-[70px]">3권</div>
              <div className="flex-1 text-left">
                <div className="font-serif font-semibold text-lg text-white tracking-tight mb-1">돈의 구조</div>
                <div className="font-sans text-[13.5px] text-white/80">회사 건강을 지표로 판단할 수 있음</div>
              </div>
              <div className="font-mono text-[11px] text-white/70 tracking-wider">25편</div>
            </div>
            <div className="group bg-white/10 border border-white/20 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-2 md:gap-5 backdrop-blur-md transition-all duration-300 hover:bg-white/20 hover:translate-x-1 hover:border-white/40 cursor-pointer">
              <div className="font-serif font-semibold text-base text-white/95 tracking-tight md:w-[70px]">4권</div>
              <div className="flex-1 text-left">
                <div className="font-serif font-semibold text-lg text-white tracking-tight mb-1">돈의 결정</div>
                <div className="font-sans text-[13.5px] text-white/80">소비·저축·대출·보험·세금·투자를 스스로 결정</div>
              </div>
              <div className="font-mono text-[11px] text-white/70 tracking-wider">31편</div>
            </div>
            <div className="group bg-white/10 border border-white/20 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-2 md:gap-5 backdrop-blur-md transition-all duration-300 hover:bg-white/20 hover:translate-x-1 hover:border-white/40 cursor-pointer">
              <div className="font-serif font-semibold text-base text-white/95 tracking-tight md:w-[70px]">5권</div>
              <div className="flex-1 text-left">
                <div className="font-serif font-semibold text-lg text-white tracking-tight mb-1">돈의 인생</div>
                <div className="font-sans text-[13.5px] text-white/80">흔들리지 않는 판단·태도·돈의 의미 정립</div>
              </div>
              <div className="font-mono text-[11px] text-white/70 tracking-wider">25편</div>
            </div>
          </div>
